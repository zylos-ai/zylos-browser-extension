...
任务卡片区分「正在操作浏览器」「浏览器待命」「已暂停」「操作已结束」。
只有浏览器指令在执行时才显示 Working；保留工作标签不会被当作仍在执行。
`pause` 暂停操作，`finish` 结束当前操作并暂时保留标签，`finalize` 清理任务并保留指定页面。
Agent 的聊天回复可能是进度说明，因此单条回复不会自动结束或关闭浏览器任务。
