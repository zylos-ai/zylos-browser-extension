...
| **引用选中文字**   | 在网页正文中选中文字，输入框上方显示来源网站图标；悬停查看内容，点击 × 移除，发送时附给 Agent。没有图标时显示默认网页图标，最多附带 4,000 字符。 |
...
选区引用来自主页面正文，不读取输入框或编辑器中的选区。大范围选区跨过这些控件时，保留前后选中的普通文字，跳过控件内容。标签跟随网页实际选区：取消选中后自动消失；发送成功、点击 ×、切换标签或刷新页面时也会清除。进入插件输入框时，只要网页仍保留选区，就可以随消息发送。它通过消息的 `message.content` 传输，读取选区无需接管浏览器。选区、图片和文件采用统一的[附件协议](docs/ATTACHMENTS.md)；目前界面提供选区引用，文件上传入口暂未开放。
...
| **[Browser Remote](https://github.com/zylos-ai/zylos-browser-remote)** | 连接插件与 Agent，转发请求和结果，处理聊天投递与图片、文件附件。 |
...
工具定义和循环指南只有插件这一份。Remote 只做请求关联、首次 C4 投递、附件转换和诊断记录。后续页面状态直接作为决策命令的返回值交给 Agent，不再逐轮经过 C4 队列。插件和 Remote 需同时支持决策协议；更新后重启 Remote 并重新加载插件。消息采用 `message`（用户内容）、`context`（页面环境）、`execution`（执行信息）三个部分；首轮发完整内容，后续通过消息 ID 关联。详见 [通信协议](docs/EXTENSION-WEBSOCKET-PROTOCOL.md) 和 [决策指南](agent/decision-guide.md)。实际速度仍取决于模型响应和网页加载。
