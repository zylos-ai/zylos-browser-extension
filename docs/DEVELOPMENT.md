...
聊天中会在首个浏览器调用处插入执行步骤条，显示工具名称、简短目标、排队/执行中/成功/失败状态和耗时。
调用进入 `IdempotencyCache` 时记录排队，在实际开始调度时记录执行中，RPC 返回后记录结果。
收到 Agent 最终回复后自动收起；点击摘要可展开。进度消息和 `finish` 不提前结束整轮记录。
手动停止显示已停止；断线或后台重启会把未确认的执行标为结果未确认，避免遗留永久运行状态。
输入框右上方的浮动画面提供查看标签页和停止按钮。普通聊天不生成空步骤条或预览。
...
聊天外的独立 `info` / `describe` 探测不产生任务记录。重试复用结果会注明，并不会再次执行操作。
步骤随聊天保存在 `remoteChatLog`，清空聊天会一起删除。只保存名称、目标提示、状态、错误码和时间，
不复制表单输入、完整参数、页面输出或截图 Base64。单轮保留最近 200 步，整个聊天保留最近 2,000 步；
超过上限仍保留数量与失败统计，并在展开时说明省略数量。
